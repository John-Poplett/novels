---
title: 'Analyzing the "Success with Style" novels dataset with the text2vec package'
...
output:
  html_document:
    ...
    toc_depth: 3
vignette: >
  %\VignetteIndexEntry{Analyzing the "Success with Style" novels dataset with the text2vec Package}
...
dim(dtm_train)
identical(rownames(dtm_train), train$title)
```
...
NFOLDS = 4
t1 = Sys.time()
glmnet_classifier = cv.glmnet(x = dtm_train, y = train[['response']],
                              family = 'binomial',
                              # L1 penalty
                              alpha = 1,
...
# turn off progressbar because it won't look nice in rmd
it_test = itoken(it_test, ids = test$title, progressbar = FALSE)

dtm_test = create_dtm(it_test, vectorizer)

preds = predict(glmnet_classifier, dtm_test, type = 'response')[,1]
glmnet:::auc(test$response, preds)
```
```{r, echo=FALSE}
...
t1 = Sys.time()
glmnet_classifier = cv.glmnet(x = dtm_train_tfidf, y = train[['response']],
                              family = 'binomial',
                              alpha = 1,
                              type.measure = "auc",
                              nfolds = NFOLDS,
